...
class CustomerCartLoader {
  ...
  constructor(actionParameters) {
    const loadingFunction = keys => {
      return Promise.resolve(
        keys.map(key => {
          console.debug(`--> Fetching customer cart details with id ${key}`);
          return this.getCustomerCart(actionParameters).catch(error => {
            console.error(
              `Failed loading customer cart ${key}, got error ${JSON.stringify(
                error,
                null,
                0
              )}`
  ...
  /**
   * Method used to get customer cart details from hybris
   * @param {Object} [parameters.graphqlContext] The optional GraphQL execution context passed to the resolver.
   * @returns {Promise} a promise resolves and return customer cart details.
   */
  getCustomerCart(actionParameters) {
    const {
      customerId,
      bearer,
      HB_API_BASE_PATH,
    ...
    const uri = `${HB_PROTOCOL}://${HB_API_HOST}${HB_API_BASE_PATH}${HB_BASESITEID}/users/${customerId}/carts?fields=FULL`;
