...
class CustomerOrderDetailsLoader {
  /**
  ...
  constructor(actionParameters) {
    /** The loading function: "keys" is an Array of order details */
    let loadingFunction = keys => {
      return Promise.resolve(
        keys.map(key => {
          console.debug(
            '--> Performing a get customer order with ' +
              JSON.stringify(key, null, 0)
          );
          return this._customerOrder(actionParameters).catch(error => {
            console.error(
              `Failed loading Customer Order ${JSON.stringify(
                key,
                null,
                0
  ...
  _customerOrder(actionParameters) {
    const {
      ...
      HB_PROTOCOL,
      HB_BASESITEID,
    } = actionParameters.actionParameters.context.settings;
    /**
     * orderCode
     */
    let code = actionParameters.code;
    ...
    const uri = `${HB_PROTOCOL}://${HB_API_HOST}${HB_API_BASE_PATH}${HB_BASESITEID}/users/${customerId}/orders/${code}?fields=FULL&access_token=${bearer}`;
...
module.exports = CustomerOrderDetailsLoader;
