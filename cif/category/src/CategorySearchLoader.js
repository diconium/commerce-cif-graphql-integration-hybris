    ...
    const loadingFunction = categories => {
      return Promise.resolve(
        categories.map(category => {
          console.debug(`--> Fetching category data ${category}`);
          return this.__getCategory(category, actionParameters).catch(error => {
            console.error(
              `Failed loading category ${category}, got error ${JSON.stringify(
                error.message,
                null,
                0
              )}`
...
module.exports = CategorySearchLoader;
